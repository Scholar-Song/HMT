...
def train_epoch(LSTM, train_target_dl, t0=t0):
    LSTM.train()
    train_loss = 0
    n = 0
...
def eval_epoch(LSTM, validation_dl, t0=t0):
    LSTM.eval()
    eval_loss = 0
    n = 0
    with torch.no_grad():
...
for e, epoch in enumerate(range(epochs)):
    ...
    l_train = train_epoch(GRU, train_data_dl)
    train_loss.append(l_train)

    # l_eval = eval_epoch(LSTM,validation_dl)
    # eval_loss.append(l_eval)

    Rp,mae,mse,mape,tic= test_epoch(GRU, test_data_dl)
...
gru_df = pd.DataFrame({"exercise_name":exercise_name,"model":"GRU","Epoch":mse_min_index,"Rp_result":Rp_result,"mae_result":mae_result,\
...
gru_df.to_csv(file_name+".txt",sep="\t",mode="a",header=False)
#gru_df.to_excel(file_name+".xlsx",mode="a")
# In[ ]:
n_plots = 5
with torch.no_grad():
    GRU.eval()
