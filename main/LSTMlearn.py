...
class LSTM_Time_Series(torch.nn.Module):
    def __init__(self, input_size=2, embedding_size=2, kernel_width=1, hidden_size=512):
        ...
        self.lstm = torch.nn.LSTM(embedding_size, hidden_size, batch_first=True,bidirectional=False)
...
def test_epoch(LSTM, test_dl, t0=t0):
...
for e, epoch in enumerate(range(epochs)):
    ...
    l_train = train_epoch(LSTM, train_data_dl)
    train_loss.append(l_train)

    # l_eval = eval_epoch(LSTM,validation_dl)
    # eval_loss.append(l_eval)

    Rp,mae,mse,mape,tic= test_epoch(LSTM, test_data_dl)
...
lstm_df = pd.DataFrame({"exercise_name":exercise_name,"model":"LSTMnet","Epoch":mse_min_index,"Rp_result":Rp_result,"mae_result":mae_result,\
...
lstm_df.to_csv(file_name+".txt",sep="\t",mode="a",header=False)
#lstm_df.to_excel(file_name+".xlsx",mode="a")
# In[ ]:
