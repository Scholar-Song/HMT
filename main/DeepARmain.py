...
from Model.DeepARfromLSTM import DeepAR_Time_Series
...
# In[ ]:
model = DeepAR_Time_Series().cuda()
def gaussian_mixture_loss(y, loc, scale, weight):
    mix = distributions.Categorical(weight)
    dist = distributions.Normal(loc, scale)
    log_prob = mix.log_prob(torch.zeros_like(y).unsqueeze(-1)) + dist.log_prob(y.unsqueeze(-1))
    return -torch.mean(torch.logsumexp(log_prob, dim=-1))
def train_epoch(model, train_target_dl, t0=t0):
    ...
    for step, (x, y, _) in enumerate(train_data_dl):
        ...
        optimizer.zero_grad()
        # loc, scale, weight = model(x.float(), y.float())
        # loss = gaussian_mixture_loss(y.float(), loc, scale, weight)
        output = model(x,y)
